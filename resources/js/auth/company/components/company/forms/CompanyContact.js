...
import {SELECT_REGIONS} from '../../../../../home/state/HomeConstants'
import {STORE_CONTACT} from '../../../../state/AuthConnstant'
...
function CompanyContact(props){
    ...
    const {inputs, handleInputChange} = UseForm({owner_id:'',email:'',phone:"",region_id:"",city_id:''}); 
    ...
    const response = useSelector(state=>state.authReducer.contactReducer.response);
    ...
    const handleSubmit =event=>{
        ...
        console.log(inputs)
        dispatch(store('/api/contact',inputs,STORE_CONTACT))
        setLoading(true)
        setSubmitted(true)
        setDone(true)
        ...
        <div>
            {
                ...
                ?
                    (
                        <CompanyFinance company={props.company}/>
                    )
                :
                    (
                        ...
                        <Typography style={{color:green[500],marginBottom:15}}>
                         {`Company is registered successfully. Now tell us contact address for company ${props.company.name}`}
                        </Typography>
